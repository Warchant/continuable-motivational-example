...
namespace boost::asio {
  ...
  // asio constructs an async_result<> instance from the yield_handler
  // specified by handler_type<>::type. A particular asio async method
  // constructs the yield_handler, constructs this async_result specialization
  // from it, then returns the result of calling its get() method.
  template <typename T>
  ...
  // When 'yield' is passed as a completion handler which accepts no
  // parameters, use yield_handler<void>.
