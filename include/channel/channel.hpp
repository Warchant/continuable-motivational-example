...
namespace appbase {
   ...
   *
   * This removes the need to tightly couple different plugins in the
   * application for the use-case of sending data around
   *
   * Data passed to a channel is *copied*, consider using a shared_ptr if the
   * use-case allows it
   *
   ...
   public:
    ...
    class handle {
     ...
     private:
       ...
       */
      explicit handle(handle_type &&_handle) : _handle(std::move(_handle)) {}
     ...
     *
     * This method is only available when the DispatchPolicy is copy
     * constructible due to implementation details
     *
     * @param policy - the DispatchPolicy to copy
     */
    auto set_dispatcher(const DispatchPolicy &policy)
        -> std::enable_if_t<std::is_copy_constructible<DispatchPolicy>::value,
                            void> {
